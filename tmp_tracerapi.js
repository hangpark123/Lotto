...
var tracer_domain="tracer.dhlottery.co.kr"; // TRACER 호출 도메인
var tracer_cookie_domain='dhlottery.co.kr';   // 서비스 도메인 변경
//var tracer_cookie_domain='localhost';   // 서비스 도메인 변경
var tracer_info_url='../tracer/info.jsp';

var dev_real_check="DEV"; // 개발자 도구 탐지 여부 DEV = 탐지 안함, PROD = 탐지 함 (개발자 도구 활성화 시 차단 페이지로 이동)
...
function callTracerApiRejectData(host, ip, loginId, port, pageUrl, sendThis) {
    ...
    $.ajax({
        type: 'POST',
        url: service_type+'://'+tracer_domain+':48081/TRACERAPI/checkBotIp.do',
        data: {
            host:host,
            ip:ip,
            port:port,
...
function callTracerInit(host, ip, loginId, port, pageUrl, sendThis) {
	...
	if ( this_reject == 'E' ) {
		...
		$.ajax({
			type: 'POST',
			url: service_type+'://'+tracer_domain+':48081/TRACERAPI/inputQueue.do',
			timeout: 3000,
			data: {
				host:host,
				ip:ip,
...
function callTracerApiOutput(host, ip, loginId, port, pageUrl, sendThis) {
    ...
    $.ajax({
        type: 'POST',
        url: service_type+'://'+tracer_domain+':48081/TRACERAPI/outQueue.do',
        data: {
            host:host,
            ip:ip,
            loginId:loginId,
