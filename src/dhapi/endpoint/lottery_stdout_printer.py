...
class LotteryStdoutPrinter:
    def print_result_of_assign_virtual_account(self, 전용가상계좌, 결제신청금액, 은행명=None, 예금주=None):
        console = Console()

        console.print("✅ 가상계좌를 할당했습니다.")
        console.print("❗️입금 전 계좌주 이름을 꼭 확인하세요.")
        if 은행명 or 예금주:
            table = Table("은행명", "전용가상계좌", "결제신청금액", "예금주")
            table.add_row(은행명 or "-", 전용가상계좌, 결제신청금액, 예금주 or "-")
        else:
